...
export default function CalendarPage() {
  ...
  const { data: reservationData } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `/api/reservations/ob_reservations/reservation`,
      );
      const json = await res.json();
  ...
  // Every "yyyy-MM-dd" an OB trip covers (a trip can span several days).
  function tripDays(res: any): string[] {
    const days: string[] = [];
    const d = new Date(res.time_from);
    d.setHours(0, 0, 0, 0);
    const end = new Date(res.time_to);
    while (d <= end) {
  ...
  // OB has many vehicles, so a day is never "fully occupied" the way a hall is.
  const fullyOccupiedDates = useMemo(() => new Set<string>(), []);

  // Count of active OB trips per "yyyy-MM-dd" date
  ...
  // Map OB trips into react-big-calendar's { title, start, end } event shape
  const calendarEvents = useMemo(() => {
    const allReservations = reservationData?.data ?? [];

    return allReservations.filter(isActiveReservation).map((res: any) => ({
      id: res.ob_id,
      title: `${res.destination} - ${res.purpose}`,
      start: new Date(res.time_from),
      end: new Date(res.time_to),
      resource: res,
    }));
  }, [reservationData]);
